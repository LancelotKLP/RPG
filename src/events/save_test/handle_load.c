...
static void parse_save_file(FILE *file, rpg_t *rpg)
{
    ...
    if (fscanf(file, "posx=%f\nposy=%f\nquest=%d\nrebuilt=%d\nmap=%d\n"
        "viewx=%f\nviewy=%f\nhp=%f\nskin=%d\nskltx=%f\nsklty=%f\nsklt=%d\n"
        "senseix=%f\nsenseiy=%f\nsenseim=%d\nhunterx=%f\nhuntery=%f\n"
        "hunterm=%d\nsword=%d\n" "spear=%d\n", &t.pos.x, &t.pos.y, &t.q, &t.r,
        &t.m, &t.v.x, &t.v.y, &t.hp, &t.w, &t.sk.x, &t.sk.y, &t.ska, &t.se.x,
        &t.se.y, &t.sem, &t.ht.x, &t.ht.y, &t.hm, &t.sw, &t.sp) != 20) {
        fprintf(stderr, "ERROR: you nasty cheater...\n");
        fprintf(stderr, "don't modify this file ^_^\n");
        return;
    }
    printf(COLOR_BOLD"load : %s%sSUCCESS\n"COLOR_RESET, COLOR_RESET,
        COLOR_GREEN);
    printf(COLOR_RESET);
